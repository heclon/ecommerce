...
import { components, CSSObjectWithLabel, MultiValue, SingleValue } from 'react-select'
...
export const CustomCreatableSelect = ({
  ...
  const onChange = (newValue: MultiValue<Option> | SingleValue<Option>) => {
    return isMulti
      ? form.setFieldValue(
          field.name,
          (newValue as MultiValue<Option>).map((n) => n.value)
        )
      : form.setFieldValue(field.name, (newValue as Option)?.value)
  }

  const value = React.useMemo(() => {
    return isMulti
      ? field.value?.map((selectedOption: string) => options.find((o) => o.value === selectedOption)).filter(Boolean)
  ...
  return (
    <CreatableSelect
      ...
      onCreateOption={(newValue: string) => {
        ...
        return isMulti ? onChange([...value, newOption]) : onChange(newOption)
      }}
